...
time_resolved_rna_matrix_intestine <- time_resolved_rna %>%
  select(-wormbase_gseq, -wikigene_name, -LENGTH) %>%
  remove_rownames() %>%
  arrange(wbps_gene_id) %>%
  filter(wbps_gene_id %in% intestine_gene_list$WBGeneID) %>%
...
```{r}
time_resolved_rna_matrix_intestine_scaled <- t(apply(unlist(time_resolved_rna_matrix_intestine), 1, scale))
colnames(time_resolved_rna_matrix_intestine_scaled) <- colnames(time_resolved_rna_matrix_intestine)
```

Store index of relavent genes for row annotations

```{r}
gene_index <- c(
  which(time_resolved_rna_intestine_df$wikigene_name == "end-1"),
  which(time_resolved_rna_intestine_df$wikigene_name == "elt-2"),
  which(time_resolved_rna_intestine_df$wikigene_name == "elt-7"),
  which(time_resolved_rna_intestine_df$wikigene_name == "elt-4"),
  which(time_resolved_rna_intestine_df$wikigene_name == "pqm-1")
)
gene_index_names <- c("end-1", "elt-2", "elt-7", "elt-4", "pqm-1")

time_resolved_rna_intestine_df %>% filter(wikigene_name %in% gene_index_names)
```


```{r}
Boeck_intestine_RNA <- Heatmap(time_resolved_rna_matrix_intestine_scaled,
        cluster_columns = FALSE,
        show_row_names = FALSE,
        row_km = 5) +
  rowAnnotation(foo = anno_mark(gene_index, labels = gene_index_names))
Boeck_intestine_RNA
# pdf(file = "./03_plots/200720_Boeck_RNA_Intestine.pdf", width = 7, height = 7)
# Boeck_intestine_RNA
# dev.off()
```

Filter heatmap for only transcription factors

```{r}
time_resolved_rna_matrix_intestine_scaled_TFONLY <-
  matrix_select(time_resolved_rna_matrix_intestine_scaled, wTF3.0$WBGeneID)
...
Heatmap(
  ...
  rowAnnotation(foo = anno_mark(
    at = c(
      which(
        rownames(time_resolved_rna_matrix_intestine_scaled_TFONLY) == "WBGene00001250"
      ),
      which(
        rownames(time_resolved_rna_matrix_intestine_scaled_TFONLY) == "WBGene00001252"
      ),
      which(
        rownames(time_resolved_rna_matrix_intestine_scaled_TFONLY) == "WBGene00001310"
      ),
      which(
        rownames(time_resolved_rna_matrix_intestine_scaled_TFONLY) == "WBGene00004096"
      ),
      which(
        rownames(time_resolved_rna_matrix_intestine_scaled_TFONLY) == "WBGene00015981"
      )
    ),
    labels = c("elt-2", "elt-4", "end-1", "pqm-1", "elt-7")
  ))
```
...
chip_rna_df <-
  ...
  right_join(time_resolved_rna, by = c("feature" = "wbps_gene_id")) %>% 
...
```{r}
gut_chip_rna_df <- chip_rna_df %>% semi_join(intestine_gene_list, c("feature" = "WBGeneID")) 
gut_chip_rna_matrix <- gut_chip_rna_df %>% select(emb_4cell:gonad) %>% as.matrix() 
BoeckRNA_ELT2_chip <- Heatmap(row_scale(gut_chip_rna_matrix),
        name = "Boeck Time Resolved RNA",
        row_split = gut_chip_rna_df$k4labels,
        row_title = NULL,
        cluster_columns = FALSE) +
  rowAnnotation(ELT2_peak = gut_chip_rna_df$k4labels, 
...
# pdf(file = "./03_plots/200720_Boeck_RNA_ELT2ChIP_Patterns.pdf", width = 7, height = 7)
# BoeckRNA_ELT2_chip
# dev.off()
```
