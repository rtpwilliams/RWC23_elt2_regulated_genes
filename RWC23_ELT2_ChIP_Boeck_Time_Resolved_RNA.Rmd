...
```{r}
time_resolved_rna <- read_delim(file = "./02_Data/9_Boeck_et_al_2016_time-resolved_transcriptome/wormbase_genes.readcounts_dcpm_per_sample.txt", delim = " ", quote = )
...
Subset the `time_resolved_rna` dataset for `dcpm` columns

```{r}
time_resolved_rna %>% select(
  wbps_gene_id,
  matches(".*EE.*dcpm"),
  matches(".*EMB.*dcpm"),
  matches(".*L1.*dcpm"),
