...
names(elt2_peaks)[names(elt2_peaks)=="L1L3"] <- "L1"
names(elt2_peaks)[names(elt2_peaks)=="...12"] <- "L3"
...
```{r}
any_pairwise_subset <- matrix_select(wt_elt2_counts, union_elt2elt7_DE$WBGeneID)
...
any_pairwise_subset_rownorm <- row_normalize_matrix(any_pairwise_subset)

myPheatmap(any_pairwise_subset_rownorm,
           title = "Genes Significantly Differentially Expressed In Any
...
```{r}
any_pairwise_subset_Zscore <- row_zscore_matrix(any_pairwise_subset)

# remove columns with NA
any_pairwise_subset_Zscore <- any_pairwise_subset_Zscore[complete.cases(any_pairwise_subset_Zscore), ]

unique(is.na(any_pairwise_subset_Zscore))

myPheatmap(any_pairwise_subset_Zscore,
           title = "Genes with Significant DE In any Pairwise Comparisons
           Row Z Score Normalized",
           rowspace = 3)
```
...
my_row_anno <- data.frame(elt2_L1_bound = ifelse(test = rownames(any_pairwise_subset_Zscore) %in% elt2_L1_peaks$WBGeneID, yes = "bound", no = "not.bound"))


rownames(my_row_anno) <- rownames(any_pairwise_subset_Zscore)

ann_colors = list(
  elt2_L1_bound = c(bound = "green", not.bound = "black")
)

pheatmap(any_pairwise_subset_Zscore, 
         ...
         annotation_colors = ann_colors,
           ...
           border_color = NA,
           cutree_rows = 3,
         main = "Genes with Significant DE In Any Pairwise Comparisons
           Row Z Score Normalized",
         width = 6,
         height = 6)#,
         #file = "./03_plots/200406_Any_DE_Genes_Elt2_Elt7_rowZscore_Bound_Annotation.pdf")
...
```{r}
any_pairwise_subset_Zscore_bindStatus <- cbind(any_pairwise_subset_Zscore, elt2Bound = ifelse(test = rownames(any_pairwise_subset_Zscore) %in% elt2_L1_peaks$WBGeneID, yes = 1, no = 0))

myPheatmap(any_pairwise_subset_Zscore_bindStatus,
...
```{r}
elt2bound_any_pairwise_subset_Zscore <- matrix_select(any_pairwise_subset_Zscore, elt2_L1_peaks$WBGeneID)

myPheatmap(elt2bound_any_pairwise_subset_Zscore,
           title = "Differential Expression of ELT-2 Bound Genes
           Subset: Differentially expressed in any pairwise comparisons
           Z Score Normalized",
           rowspace = 3)
```
...
```{r}
elt2bound_any_pairwise_subset_Zscore_TF <- matrix_select(elt2bound_any_pairwise_subset_Zscore, wTF3.0$WBGeneID)

elt2bound_any_pairwise_subset_Zscore_TF <- id2name(elt2bound_any_pairwise_subset_Zscore_TF)

pheatmap(elt2bound_any_pairwise_subset_Zscore_TF,
         cluster_rows = TRUE,
         cluster_cols = FALSE,
         show_rownames = TRUE,
         border_color = NA
...
Subset count matrix for presence in union of any pairwise comparisons.  

```{r}
any_pairwise_subset <- matrix_select(wt_elt2_elt7double_counts, union_elt2elt7_DE$WBGeneID)
...
pheatmap(elt2bound_any_pairwise_subset_Zscore_TF,
         ...
         Subset of Significant DE In any Pairwise Comparisons
...
bound_expressed_annotation <- cbind(my_row_anno, 
                                    LE.Intestine = ifelse(test = rownames(any_pairwise_subset_Zscore) %in% spencer_LE_subset$spencer_LE_ID, yes = "enriched", no = "depleted"),
                                    L2.Intestine = ifelse(test = rownames(any_pairwise_subset_Zscore) %in% spencer_L2_subset$spencer_L2_ID, yes = "enriched", no = "depleted"))
...
dim(any_pairwise_subset_Zscore)
pheatmap(any_pairwise_subset_Zscore, 
         ...
         width = 6,
         height = 6,
         file = "./03_plots/200411_any_DE_Genes_Elt2_Elt7_rowZscore_Bound_Expressed_Annotation.pdf")


```


Collapse ELT-2 binding pattern annotation per gene.  
...
per_gene_boundclass_counts <- elt2_peaks %>% 
  select(WBGeneID, cluster.description) %>% 
  table() %>% 
  as.data.frame() %>% 
  pivot_wider(id_cols = WBGeneID, names_from = cluster.description, values_from = Freq)

head(per_gene_boundclass_counts)
```

Add row annotation based on ELT-2 binding patterns.  

```{r}
bound_expressed_class_annotation <- 
  bound_expressed_annotation %>% 
  rownames_to_column(var = "WBGeneID") %>%
  left_join(per_gene_boundclass_counts, by = "WBGeneID") %>% 
  replace_na(list(NotChanging = 0, L3high = 0, Increasing = 0, PostEmbryonic = 0, LEspecific = 0)) %>%
  column_to_rownames(var = "WBGeneID")

head(bound_expressed_class_annotation)

bound_expressed_class_ann_colors <- list(
  elt2_L1_bound = c(bound = "green", not.bound = "black"), 
  LE.Intestine = c(enriched = "blue", depleted = "white"), 
  L2.Intestine = c(enriched = "blue", depleted = "white")
  )
...
pheatmap(any_pairwise_subset_Zscore, 
         annotation_row = bound_expressed_class_annotation,
         annotation_colors = bound_expressed_ann_colors,
           cluster_cols = FALSE, 
           cluster_rows = TRUE, 
           show_rownames = FALSE, 
         ...
         width = 6,
         height = 9,
         filename = "./03_plots/200410_All_DE_Genes_ELT2_ELT7_rowZscore_bound_expressed_class_annotation_full_assignment.pdf")

rownames(bound_expressed_class_annotation) %in% rownames(any_pairwise_subset_Zscore) %>% unique
```

Extract clusters.  

```{r}
out <- pheatmap(any_pairwise_subset_Zscore, 
         annotation_row = bound_expressed_class_annotation,
         annotation_colors = bound_expressed_ann_colors,
           cluster_cols = FALSE, 
           cluster_rows = TRUE, 
...
expression_cluster <- sort(cutree(out$tree_row, k = 7))

expression_cluster %>% unique

annotations <- bound_expressed_class_annotation %>% add_column(expression_cluster = cutree(out$tree_row, k = 7)) 
...
pheatmap(any_pairwise_subset_Zscore, 
         ...
         annotation_colors = bound_expressed_ann_colors,
           ...
           border_color = NA,
         cutree_rows = 7,
         filename = "./03_plots/200413_cluster_assignment_sanity_check.pdf")

annotations %>% rownames_to_column(var = "WBGeneID") %>% filter(expression_cluster == 7) %>% select("WBGeneID")
```


Load in clustering datafrom and rlog counts dineen et al.  

```{r}
dineen_nishimura_counts <- read_xlsx(path = "./01_input/Table_S2_rlog_Stabilized_Read_Counts.xlsx", sheet = "Sheet1")

dineen_nishimura_counts_matrix <- dineen_nishimura_counts %>% column_to_rownames(var = "WBGeneID")  %>% data.matrix()

dineen_nishimura_counts_matrix[1,1] %>% class

row_normalize_matrix(dineen_nishimura_counts_matrix) %>% head



dineen_nishimura_counts_matrix_any_pairwise <- matrix_select( dineen_nishimura_counts_matrix, union_elt2elt7_DE$WBGeneID)

dineen_nishimura_counts_matrix_any_pairwise_row_normalize <- row_normalize_matrix(dineen_nishimura_counts_matrix_any_pairwise)

is.na(dineen_nishimura_counts_matrix_any_pairwise_row_normalize) %>% unique

myPheatmap(dineen_nishimura_counts_matrix_any_pairwise_row_normalize)
```
