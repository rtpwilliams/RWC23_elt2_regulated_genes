...
- cluster set assignments derived from heatmap do not match when plotted back to the heatmap
...
unique(rownames(dynamic_counts_matrix_scaled_ascend) == chip_annotation$rowname)
...
```{r}
kclus <- kmeans(dynamic_counts_matrix_scaled_bound_only, 5)
bound_only_sets <-
  data.frame(
...
```{r}
Ha_bound_only <-
  RNA_heatmap(dynamic_counts_matrix_scaled_bound_only,
              bound_only_sets$set) +
...
bound_only_annotation_ascend <-
  bound_only_annotation %>% arrange(rowname)
