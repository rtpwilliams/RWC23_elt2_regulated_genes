...
```{r}
variance_per_gene %>% filter(set == "SET1", external_gene_id == "ets-4")
```
SET1 strains to order: OP509 (ets-4).  
...
```{r}
variance_per_gene %>% filter(set == "SET3", external_gene_id == "pqm-1")
```
...
```{r}
variance_per_gene %>% filter(set == "SET4", external_gene_id == "mtl-2")
```
...
genes_to_test <- variance_per_gene %>% filter(external_gene_id %in% c("ets-4", "cebp-1", "him-4", "mtl-2", "pqm-1")) %>% select(wbps_gene_id, external_gene_id)
...
Ha_bound_only_test_gene <- Ha_bound_only +
  rowAnnotation(foo = anno_mark(
    at = c(
      ...
      which(
        rownames(dynamic_counts_matrix_scaled_bound_only) == genes_to_test$wbps_gene_id[5]
      )
    ),
...
```{r}
write_csv(
  (variance_per_gene %>% select(wbps_gene_id) %>% rename("Wormbase ID" = wbps_gene_id)),"./02_output/all_DE_genes.csv")
write_csv(
  (variance_per_gene %>% filter(set == "SET1") %>% select(wbps_gene_id) %>% rename("Wormbase ID" = wbps_gene_id)),"./02_output/set1_DE_genes.csv")
write_csv(
  (variance_per_gene %>% filter(set == "SET2") %>% select(wbps_gene_id) %>% rename("Wormbase ID" = wbps_gene_id)),"./02_output/set2_DE_genes.csv")
write_csv(
  (variance_per_gene %>% filter(set == "SET3") %>% select(wbps_gene_id) %>% rename("Wormbase ID" = wbps_gene_id)),"./02_output/set3_DE_genes.csv")
write_csv(
  (variance_per_gene %>% filter(set == "SET4") %>% select(wbps_gene_id) %>% rename("Wormbase ID" = wbps_gene_id)),"./02_output/set4_DE_genes.csv")
