...
# cluster analysis
howmany=2:11
names(howmany) = howmany
cl = makeClusterPSOCK(4, outfile=NULL, verbose=T)
clusterExport(cl, varlist = list("time_resolved_rna_intestine_matrix_scaled_TFONLY"))
withinSS = parLapply(cl, howmany, function(k){
  kmeans(time_resolved_rna_intestine_matrix_scaled_TFONLY, as.integer(k))$tot.withinss
})

stopCluster(cl)

plot(howmany,unlist(withinSS))
...
### ALL ###
qthreshold = quantile(rowvariances,.1)
...
hc = hclust(dist(data.whole))
# plot the hc height to get the scree
ggplot(NULL, aes(x=length(hc$height):1, y=hc$height)) +
    geom_point() + geom_line() +
    theme_bw() + labs(title="Scree Plot of hclust", x = "# of clusters", y="Height") + scale_x_continuous(limits=c(1,15)) + scale_y_continuous(limits=c(5,6))

nclust=8
Heatmap(data.whole,cluster_columns = F,row_order=hc$order,row_split=cutree(hc, nclust),show_row_names = F) +
  rowAnnotation(
...
dimnames(tab) <- list("hclusters"=1:nclust, "ELT-2"=levels(chip_rna_df$cluster.description[real&changing]))
...
```{r cluster comparisons and other exploration}
### Exclude the least changing, lower 5% of variance ###
changing = rowvariances > quantile(rowvariances,.05)

BoeckRNA_ELT2_chip_Heatmap(real & changing, "Log(RNA Timecourse), Exclude lower 5% variance")

for (clustering_method in c("ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid" )){
    BoeckRNA_ELT2_chip_Heatmap(real & changing,
                               sprintf("%s, Exclude lower 5%% variance",
                                       toupper(clustering_method)),
                               clustering_method_rows = clustering_method)-> plottedHM
    print(plottedHM)
}


BoeckRNA_ELT2_chip_Heatmap(real & changing & embryo_specific, "Log(RNA Timecourse), Exclude lower 5% variance")
...
for (clustering_method in c("ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid" )){
    BoeckRNA_ELT2_chip_Heatmap(real & changing,
                               sprintf("Euclidean, %s, variance > 1 (top 20%% data)",
                                       toupper(clustering_method)),
                               clustering_method_rows = clustering_method)-> plottedHM
    print(plottedHM)
        BoeckRNA_ELT2_chip_Heatmap(real & changing,
                               sprintf("Gower, %s, variance > 1 (top 20%% data)",
                                       toupper(clustering_method)),
                               clustering_method_rows = clustering_method,
                               clustering_distance_rows = function(x){daisy(x,"gower")}
                               )-> plottedHM
    print(plottedHM)
}
